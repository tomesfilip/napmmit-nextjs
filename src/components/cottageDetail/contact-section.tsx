      ...
      <div className="flex w-full flex-wrap gap-6 lg:grid-cols-3 lg:gap-12">
        {phoneNumber && (
          <div className="flex items-center gap-2">
            <Icon icon="Phone" className="size-6 shrink-0 fill-black" />
            <a href={`tel:${phoneNumber}`}>{phoneNumber}</a>
          </div>
        )}
        {email && (
          <div className="flex items-center gap-2">
            <Icon icon="Mail" className="size-7 shrink-0 fill-black" />
            <a href={`mailto:${email}`}>{email}</a>
          </div>
        )}
        {website && (
          <div className="flex items-center gap-2">
            <Icon icon="Link" className="size-6 shrink-0 fill-black" />
            <Link
              className="max-w-[230px] overflow-hidden text-ellipsis lg:max-w-[360px]"
              target="_blank"
              href={website}
            >
              {website}
